      ...
      <div className="hud-panel rounded-2xl p-6 max-w-lg w-full shadow-2xl animate-math-fade-in max-h-[80vh] flex flex-col">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-xs text-red-400">{new Date().toISOString().slice(0, 7)} 月間ランキング</p>
          </div>
        ...
        <div className="flex-grow overflow-y-auto space-y-2">
          {loading ? (
            <div className="text-center text-red-600 py-8 animate-pulse">読み込み中...</div>
          ) : entries.length === 0 ? (
            <div className="text-center text-red-700 py-8">
              <p className="text-sm">まだ記録がありません</p>
              <p className="text-xs mt-1 text-red-800">この単元を練習して1位を目指そう！</p>
            </div>
          ) : (
            entries.map((entry, i) => (
              <div key={i} className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${
                ...
                'bg-slate-900/30 border-red-500/5'
              }`}>
                ...
                <div className="flex-grow min-w-0">
                  ...
                  {entry.studentLabel && <p className="text-xs text-red-500">{entry.studentLabel}</p>}
                </div>
                <div className="text-right">
                  <p className="text-lg font-bold font-mono text-red-300">{entry.score}</p>
                  <p className="text-[10px] text-red-600">pts</p>
                </div>
              </div>
            ))
          )}
        ...
        <button onClick={onClose} className="mt-4 btn-tactical w-full py-3 rounded-lg font-bold text-red-400">
          閉じる
        </button>
      </div>
    </div>
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-start sm:justify-center p-3 sm:p-6 text-white overflow-y-auto">
      ...
      <header className="w-full max-w-5xl text-center mb-4 sm:mb-8 flex-shrink-0">
        ...
        <p className="text-red-400 text-xs sm:text-sm font-bold">分野を選んで問題を解こう</p>
      </header>
      ...
      <main className="w-full max-w-7xl hud-panel rounded-2xl p-0 shadow-2xl relative overflow-hidden flex flex-col md:flex-row flex-grow md:flex-grow-0 md:h-[65vh] min-h-0">
        ...
        <div className="w-full md:w-1/4 bg-slate-950/60 border-b md:border-b-0 md:border-r border-red-500/10 flex md:flex-col p-3 sm:p-4 md:p-6 gap-2 md:gap-3 overflow-x-auto md:overflow-x-visible md:overflow-y-auto flex-shrink-0">
          <h2 className="text-xs font-bold text-red-400 mb-1 md:mb-2 hidden md:block">分野を選択</h2>
            ...
            <button
              ...
              className={`p-2 sm:p-3 md:p-4 text-left rounded-xl transition-all duration-300 font-bold text-xs sm:text-sm border whitespace-nowrap md:whitespace-normal flex-shrink-0
                ${selectedCategory?.name === cat.name
                  ...
                  : 'bg-transparent border-transparent text-red-700 hover:text-red-400 hover:bg-slate-900/40'
                }`}
            >
              {cat.name}
            </button>
        ...
        {selectedCategory ? (
          <div className="flex-grow overflow-y-auto custom-scrollbar p-6 md:p-10 animate-math-fade-in">
            <header className="mb-8 flex justify-between items-end border-b border-red-500/10 pb-4">
                ...
                <p className="text-xs text-red-500 font-bold hidden sm:block">{selectedCategory.groups.reduce((acc, g) => acc + g.subtopics.length, 0)} 単元</p>
            </header>
                            ...
                            {group.subtopics.map((subTopic: string) => (
                                <div key={subTopic} className="relative group">
                                  <button
                                      ...
                                      className="w-full bg-slate-950/40 border border-red-500/10 text-red-100/90 p-4 rounded-xl text-sm font-bold hover:bg-red-500 hover:text-slate-950 hover:border-white transition-all text-left relative overflow-hidden pr-12"
                                  >
                                      <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-500/20 group-hover:bg-white transition-colors"></div>
                                      <span className="relative z-10 block">{subTopic}</span>
                                  </button>
                                  {db && (
                                    <button
                                      onClick={(e) => { e.stopPropagation(); setRankingTopic(subTopic); }}
                                      className="absolute right-2 top-1/2 -translate-y-1/2 p-2 text-red-600 hover:text-amber-400 transition-colors z-10"
                                      title="ランキングを見る"
                                    >
          ...
          <div className="flex-grow flex items-center justify-center border-2 border-dashed border-red-900/20 rounded-xl m-6">
             <p className="text-red-700 animate-pulse text-sm">分野を選択してください</p>
          </div>
        )}
      </main>
